...
<?php else : // Set the Featured Image as the Background Image on Archive Pages ?>
    ...
    <article id="post-<?php the_ID(); ?>" <?php post_class(); ?> <?php
            if ( '' != get_the_post_thumbnail() ) { ?> style="background: white url(<?php echo esc_url( the_post_thumbnail_url( 'simpleasy-featured-image' ) ); ?>);" <?php } ?>>
	...
	<div class="entry-content">
		<?php
                    ...
                    if ( is_archive() || is_home() || is_front_page() || is_page_template( 'page-templates/frontpage-portfolio.php' ) ) { // Makes EVERY Post on an index page an excerpt
                        simpleasy_fancy_excerpt();
                    } else {
			the_content( sprintf(
				/* translators: %s: Name of current post. */
				wp_kses( __( 'Continue reading %s <span class="meta-nav">&rarr;</span>', 'simpleasy' ), array( 'span' => array( 'class' => array() ) ) ),
