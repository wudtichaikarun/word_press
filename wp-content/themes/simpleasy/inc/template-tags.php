...
if ( ! function_exists( 'simpleasy_posted_on' ) ) :
/**
 * Prints HTML with meta information for the current post-date/time and author.
 */
function simpleasy_posted_on() {
        global $post;
	...
	$byline = sprintf(
		...
		esc_html_x( 'By %s', 'post author', 'simpleasy' ),
	...
	//echo '<span class="byline"> ' . $byline . '</span>';
		...
		/* translators: used between list items, there is a space after the comma */
                if( 'post' === get_post_type() ) {
                    $categories_list = get_the_category_list( __( '</li><li>', 'simpleasy' ) );
...
if ( ! function_exists( 'simpleasy_entry_footer' ) ) :
/**
 * Prints HTML with meta information for the categories, tags and comments.
 */
function simpleasy_entry_footer() {
    global $post;
	...
	if ( 'post' === get_post_type() || 'jetpack-portfolio' === get_post_type() ) {
		/* translators: used between list items, there is a space after the comma */
            if( 'post' === get_post_type() ) {
		$categories_list = get_the_category_list( esc_html__( ', ', 'simpleasy' ) );
            } elseif( 'jetpack-portfolio' === get_post_type() ) {
                $categories_list = get_the_term_list( $post->ID, 'jetpack-portfolio-type', '', esc_html_x(', ', 'Used between list items, there is a space after the comma.', 'simpleasy' ), '');
            }
		if ( $categories_list && simpleasy_categorized_blog() ) {
	...
	if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		...
		comments_popup_link( esc_html__( 'Leave a comment', 'simpleasy' ), esc_html__( '1 Comment', 'simpleasy' ), esc_html__( '% Comments', 'simpleasy' ) );
		echo '</span>';
	}
}
endif;
 ...
 * @return bool
 */
function simpleasy_categorized_blog() {
	if ( false === ( $all_the_cool_cats = get_transient( 'simpleasy_categories' ) ) ) {
		...
		set_transient( 'simpleasy_categories', $all_the_cool_cats );
	}

	if ( $all_the_cool_cats > 1 ) {
		// This blog has more than 1 category so simpleasy_categorized_blog should return true.
		return true;
	} else {
		// This blog has only 1 category so simpleasy_categorized_blog should return false.
		return false;
	}
}

/**
 * Flush out the transients used in simpleasy_categorized_blog.
 */
function simpleasy_category_transient_flusher() {
	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
		return;
	}
	// Like, beat it. Dig?
	delete_transient( 'simpleasy_categories' );
}
add_action( 'edit_category', 'simpleasy_category_transient_flusher' );
add_action( 'save_post',     'simpleasy_category_transient_flusher' );



/*==============================================================================
 * simpleasy CUSTOM TAGS BELOW
 =============================================================================*/
/**
 * Fancy excerpts
 * 
 ...
 */
function simpleasy_fancy_excerpt() {
    global $post;
    if( is_archive() ) {
       the_excerpt();
        echo '<div class="continue-reading">';
        echo '<a class="more-link more-link-activated" href="' . esc_url( get_permalink() ) . '" title="' . esc_attr__( 'Continue Reading ', 'simpleasy' ) . get_the_title() . '" rel="bookmark">' . esc_html__( 'Continue Reading', 'simpleasy' ) . '</a>'; 
        echo '</div>';
    } elseif ( is_page_template( 'page-templates/page-child-pages.php' ) ) {
        the_excerpt();
         echo '<div class="continue-reading">';
        echo '<a class="continue-reading-arrow" href="' . esc_url( get_permalink() ) . '" title="' . esc_attr__( 'Continue Reading ', 'simpleasy' ) . get_the_title() . '" rel="bookmark">&rarr;</a>'; 
        echo '</div>';
...
function simpleasy_author_box() {
    ...
    if ( isset( $post->post_author ) ) {
        ...
        $author_details .= '<h3 class="author-title"><span>' . esc_html__( 'About ', 'simpleasy' );
            if ( is_author() ) $author_details .= $display_name;        // If an author archive, just show the author name
            else $author_details .= esc_html__( 'the Author', 'simpleasy' ); // If a regular page, show "About the Author"
        ...
        if ( ! is_author() ) {  // Don't show the meta info on an author archive page
            $author_details .= '<p class="author-links entry-meta"><span class="vcard">' . esc_html__( 'All posts by', 'simpleasy' ) . '<a class="fn" href="' . esc_url( $user_posts ) . '">' . $display_name . '</a></span>';

            // Check if author has a website in their profile
            if ( ! empty( $user_site ) ) 
                $author_details .= '<a class="author-site" href="' . esc_url( $user_site ) . '" target="_blank" rel="nofollow">' . esc_html__( 'Website', 'simpleasy' ) . '</a></p>';
            else $author_details .= '</p>';
        }
        
        $author_details .= '</section>';
        $author_details .= '</div>';
        $author_details .= '<p class="show-hide-author label">' . esc_html__( 'Hide', 'simpleasy' ) . '</p>';
...
function simpleasy_portfolio_index_footer() {
    $time_string = '<time class="entry-date published" datetime="%1$s">%2$s</time>';
    if ( get_the_time( 'U' ) !== get_the_modified_time( 'U' ) ) {
            $time_string = '<i class="fa fa-refresh tip"><span class="tooltip">' .
                        esc_attr__( 'First posted: ', 'simpleasy' ) .
    ...
    $project_type = get_the_term_list( get_the_ID(), 'jetpack-portfolio-type', '<span class="portfolio-entry-meta cat-links">', esc_html_x( ', ', 'Used between list items, there is a space after the comma.', 'simpleasy' ), '</span>' );
...
if ( ! function_exists( 'simpleasy_breadcrumbs' ) ) :
/**
 ...
 */
function simpleasy_breadcrumbs() {
    ...
    $home_link = '<a aria-label="' . esc_html__( 'Home', 'simpleasy' ) .'" title="' . esc_attr__( 'Home', 'simpleasy' ) .'" class="breadcrumb-home" href="' . esc_url( home_url() ) . '"><i class="fa fa-home"></i></a>' . $separator;
    
    $output .= "<div aria-label='" . esc_html__( 'You are here:', 'simpleasy' ) . "' id='$breadcrumb_id' class='$breadcrumb_class'>";
    $output .= $home_link;
 ...
 * Social Menu
 */
function simpleasy_social_menu() {
...
function simpleasy_the_post_icon() {
    echo wp_kses( simpleasy_post_icon(), array( 'span' => array( 'class' => array() ) ) );
}

/**
 * Function to show the Jetpack sharing and Likes only at the designated locations in Posts and Pages
 */
function simpleasy_jetpack_sharing() {
 ...
 */
function simpleasy_post_navigation() {
    ...
    <nav class="navigation post-navigation" role="navigation">
        <p class="screen-reader-text"><?php esc_html_e( 'Post navigation', 'simpleasy' ); ?></p>
        <div class="nav-links" data-equalizer>
                <?php
                        previous_post_link( '<div class="nav-previous" data-equalizer-watch><div class="nav-indicator">' . esc_attr_x( 'Previous Post:', 'Previous post', 'simpleasy' ) . '</div><h4>%link</h4></div>', '%title' );
                        next_post_link(     '<div class="nav-next" data-equalizer-watch><div class="nav-indicator">'     . esc_attr_x( 'Next Post:', 'Next post', 'simpleasy' ) . '</div><h4>%link</h4></div>', '%title' );
                ?>
        </div> <!-- .nav-links -->
    </nav> <!-- .navigation -->
    <?php
...
if ( ! function_exists( 'simpleasy_copyright' ) ) :
/** 
